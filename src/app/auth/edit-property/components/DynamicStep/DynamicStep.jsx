...
export default function DynamicStep({ stepData, allSteps, currentStepIndex }) {
  ...
  const handleContinue = () => {
    ...
    if (currentStepIndex < allSteps.length - 1) {
      ...
      router.push(`/auth/edit-property/${nextPath}` + (typeof window !== 'undefined' ? window.location.search : ''));
  ...
  const handleBack = () => {
    if (currentStepIndex > 0) {
      ...
      router.push(`/auth/edit-property/${prevPath}` + (typeof window !== 'undefined' ? window.location.search : ''));
    } else {
