...
const MobileSideMenu = () => {
  ...
  { name: "Find Agent", submenu: "find-agent" },
  { name: "Projects", submenu: "projects" },
  { name: "Property Services", submenu: "property-services" },
  { name: "Home Loans", submenu: "home-loans" },
  { name: "Help", submenu: "help" },
  { name: "Sign In", submenu: "sign-in" },
];
  ...
  <div className={`menu-panel ${activeMenu === title ? "active" : ""}`}>
    <div className="d-flex align-items-center border-bottom px-3 py-3">
      ...
      />
      <h6 className="m-3 my-0">{title.charAt(0).toUpperCase() + title.slice(1)}</h6>
    </div>
      ...
      {data.map((section, idx) => {
        ...
        return (
          <div key={idx} className="mb-4">
            ...
            >
              <h6 className="section-heading text-muted m-0">{section.heading}</h6>
              {section.expandable && (isOpen ? <FaMinus size={14} className="m-0" /> : <FaPlus size={14} className="m-0" />)}
            </div>
...
const renderSellSubMenu = () => (
                ...
                <ul className="list-unstyled ps-3 pt-2">
                  {section.items.map((item, j) => (
                    <li key={j} className="mb-2">
                      ...
                      {item.badge && (
                        <span className="badge bg-warning text-dark ms-2">{item.badge}</span>
                      )}
                    </li>
                  ))}
                </ul>
 ...
 return (
  <div className="position-relative overflow-hidden" style={{ height: "100vh", backgroundColor: "#fff" }}>
    {/* MAIN MENU */}
      ...
      <ul className="list-unstyled px-3 py-2">
        {menuItems.map((item, i) => (
          ...
          >
            ...
            {item.submenu && <FaChevronRight size={14} className="text-muted me-0" />}
          </li>
        ))}
      </ul>
    </div>
